                    ...
                    <div class="w-9/12 flex flex-col justify-between">
                        ...
                        <div class="w-full p-6 flex flex-col overflow-y-scroll">
                            <div v-for="message in messages" :key="message.id"
                                :class="(message.from == user.id) ? 'text-right' : ''" class="w-full mb-3 message">
                                <p :class="(message.from == user.id) ? 'messageFromMe' : 'messageToMe'"
...
export default {
    ...
    created() {
        ...
        store.dispatch('setUserStateAction', auth.user);
    },
        ...
        loadMessages: async function (userId) {
            ...
            await axios.get(`api/messages/${userId}`).then(response => {
                ...
                this.users.forEach((user, i) => {
                    if (user.id == userId) {
                        this.users[i].notification = false;
                    }
                });
            });
            ...
            if (this.userActive && this.userActive.id == e.message.from) {
                ...
                this.users.forEach((user, i) => {
                    if (user.id == e.message.from) {
                        this.users[i].notification = true;
                    }
                });
            }
        });
